...
template <typename ElemType>
void projection_synapses_add_wrapper(core::Projection<ElemType> &projection, const py::object &gen_func, size_t index)
{
    projection.add_neurons(
        ProjectionGeneratorProxy<typename core::Projection<ElemType>::SynapseParameters>(gen_func), index);
};
