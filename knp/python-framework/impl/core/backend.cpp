...
BOOST_PYTHON_MODULE(Backend)
{
    py::class_<Backend>("Backend", "The Backend class is the base class for backends.")
        ...
        .def("remove_projections", &core::Backend::remove_projections, "Remove projections with given UIDs from the backend.")
        .def("remove_populations", &core::Backend::remove_populations, "Remove populations with given UIDs from the backend.")
        .def(
            "get_current_devices", (std::vector<std::unique_ptr<Device> >(core::Backend::*)()) & core::Backend::get_current_devices,
            "Get a list of devices on which the backend runs a network.")
        .def(
            "get_current_devices", (std::vector<std::unique_ptr<core::Device> >(core::Backend::*)()) & core::Backend::get_current_devices,
            "Get a list of devices on which the backend runs a network.")
        .def("select_devices", &core::Backend::select_devices, "Select devices on which to run the backend.")
        .def(
        ...
        .def("start", (void(core::Backend::*)(core::RunPredicate)) & core::Backend::start, "Start network execution on the backend.");
}
*/
