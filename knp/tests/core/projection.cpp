...
TEST(ProjectionSuite, SynapseAddition)
{
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        ASSERT_NE(std::find_if(connections.begin(), connections.end(),
...
TEST(ProjectionSuite, SynapseRemoval)
{
    ...
    count = projection.add_synapses(total_connections,
                                    make_cyclic_generator({presynaptic_size, postsynaptic_size}, {0, 1}));
