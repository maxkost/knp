...
def get_preproc_hash(compile_args, config):
    ...
    preproc_flag = "-E"
    keep_comments_flag = config.get("preprocessor", "preserve_comments", fallback="-C")
...
def compute_cache_key(clang_tidy_call, config):
    ...
    enabled_checks = run_get_stdout([clang_tidy] + clang_tidy_args + ["--list-checks"])
...
def init_cltcache():
    cltcache_path = os.environ.get("CLTCACHE_DIR", pathlib.Path().home() / ".cltcache")
...
def cache_clang_tidy(clang_tidy_call):
    ...
    cache_results = (clt_success or config.getboolean("behavior", "cache_failure", fallback=True)) and preproc_success
    if cache_results:
