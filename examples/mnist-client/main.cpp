/**
 * A demo program for building and running MNIST-classification spiking network.
 */
    ...
    knp::core::UID &output_population_uid, std::vector<std::string> stdp_population_names = {})
{
    ...
    while (!getline(moncsv, str).eof())
    {
        if (str.substr(0, n) == ss.str())
        {
            ...
            if (neuron >= int(neuron_populations.size())) neuron_populations.resize(neuron + 1, {-1, 0});
    ...
    do
    {
        if (str.substr(0, n) == ssneu.str())
        {
            ...
            double real_number;
            ss >> neuron;
            if (neuron < int(neuron_populations.size()) && neuron_populations[neuron].first >= 0)
            {
        ...
        else if (str.substr(0, n) == sslin.str())
        {
            ...
            if (neuron < int(neuron_populations.size()) && neuron_populations[neuron].first >= 0)
            {
                ...
                if ((source < 0 ||
                     (source - 1 < int(neuron_populations.size()) && neuron_populations[source - 1].first >= 0)))
                {
