...
public:
    ...
    STDPSynapse operator()(size_t index)
    {
        ...
        STDPSynapse result;
        auto &param = std::get<knp::core::SynValue>(result);
        std::get<knp::core::NeuronIdFrom>(result) = std::get<knp::core::NeuronIdFrom>(base_synapse);
        std::get<knp::core::NeuronIdTo>(result) = std::get<knp::core::NeuronIdTo>(base_synapse);
        param.weight_ = std::get<knp::core::SynValue>(base_synapse).weight_;
        param.output_type_ = std::get<knp::core::SynValue>(base_synapse).output_type_;
        param.delay_ = std::get<knp::core::SynValue>(base_synapse).delay_;
    ...
    do
    {
        ...
        else if (str.substr(0, n) == sslin.str())
        {
            ...
            if (neuron < static_cast<int>(neuron_populations.size()) && neuron_populations[neuron].first >= 0)
            {
                ...
                int source;
                auto &syn_p = std::get<knp::core::SynValue>(syn);
                                    ...
                                    neuron_populations[source - 1].first >= 0)))
                {
                    ...
                    std::get<knp::core::NeuronIdTo>(syn) = uint32_t(neuron_populations[neuron].second);
                    std::get<knp::core::NeuronIdFrom>(syn) =
